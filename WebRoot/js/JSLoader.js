var GLOBAL = this, JSLoader = function(config) {
	...
	try {
		if (_localStorage) {
			var localVersion = _localStorage.getItem('versionTextStr'), winxhr = getXHR();
			winxhr.open('GET', 'version.txt', false);
			winxhr.send('');
			if (winxhr.status == 200) {
				window._enable_localStorage = true;
				var serviceVersion = winxhr.responseText;
	...
	var jsloader = function() {
		...
		loaderChain[1].resolveMany( [ 'freequery.lang.lang', 'freequery.lang.domutils',
				...
				'freequery.lang.CustomEvent', 'freequery.control.logger' ]);
		// --StaticLoaderEnd-- //
